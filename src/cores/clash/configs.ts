                ...
                for (const host of hosts) {
                    ...
                    if (outbound) {
                        ...
                        if (domain === customDomain) {
                            tagGroup['💦 MatiX Best Ping D 🚀'].push(tag);
                        } else {
                            tagGroup['💦 MatiX Best Ping 🚀'].push(tag);
                        }
                        ...
                        if (isChain) {
                            ...
                            if (domain === customDomain) {
                                tagGroup['💦 MatiX 🔗 Best Ping D 🚀'].push(chainTag);
                            } else {
                                tagGroup['💦 MatiX 🔗 Best Ping 🚀'].push(chainTag);
                            }
                        }
...
export async function getClWarpConfig(isPro: boolean): Promise<Response> {
    ...
    const tagGroup: TagGroup = {
        [`💦 MatiX Warp ${proSign}- Best Ping 🚀`]: [],
        [`💦 MatiX WoW ${proSign}- Best Ping 🚀`]: []
    };

    warpEndpoints.forEach((endpoint, index) => {
        const warpTag = `💦 MatiX ${index + 1}. Warp ${proSign}🇮🇷`;
        tagGroup[`💦 MatiX Warp ${proSign}- Best Ping 🚀`].push(warpTag);

        const wowTag = `💦 MatiX ${index + 1}. WoW ${proSign}🌍`;
        tagGroup[`💦 MatiX WoW ${proSign}- Best Ping 🚀`].push(wowTag);
