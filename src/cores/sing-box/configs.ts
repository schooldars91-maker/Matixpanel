...
export async function getSbWarpConfig(): Promise<Response> {
    ...
    const tagGroup: TagGroup = {
        '💦 MatiX Warp - Best Ping 🚀': [],
        '💦 MatiX WoW - Best Ping 🚀': []
    };

    warpEndpoints.forEach((endpoint, index) => {
        const warpTag = `💦 MatiX ${index + 1}. Warp 🇮🇷`;
        tagGroup['💦 MatiX Warp - Best Ping 🚀'].push(warpTag);

        const wowTag = `💦 MatiX ${index + 1}. WoW 🌍`;
        tagGroup['💦 MatiX WoW - Best Ping 🚀'].push(wowTag);
