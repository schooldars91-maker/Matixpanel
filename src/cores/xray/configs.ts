...
async function addBestPingConfigs(
    ...
    const remark = `💦 MatiX ${chainSign}Best Ping ${configType}🚀`;
    const outbounds = [
        ...chainOutbounds,
        ...proxyOutbounds
    ];
...
async function addBestFragmentConfigs(
    ...
    const config = await buildConfig(
        `💦 MatiX ${chainSign}Smart Fragment 🧠`,
        outbounds,
        true,
        isChain,
        false,
...
export async function getXrWarpConfigs(
    ...
    for (const [index, endpoint] of warpEndpoints.entries()) {
        ...
        const warpConfig = await buildConfig(
            `💦 MatiX ${index + 1} - Warp${proIndicator}🇮🇷`,
            [warpOutbound],
            false,
            false,
            false,
        ...
        const wowConfig = await buildConfig(
            `💦 MatiX ${index + 1} - WoW${proIndicator}🌍`,
    ...
    const warpBestPing = await buildConfig(
        `💦 MatiX Warp${proIndicator}- Best Ping 🚀`,
        [...proxies],
        true,
        false,
        false,
    ...
    const wowBestPing = await buildConfig(
        `💦 MatiX WoW${proIndicator}- Best Ping 🚀`,
        [...chains, ...proxies],
        true,
        true,
        false,
