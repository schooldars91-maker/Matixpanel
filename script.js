...
const i18nDict = {
    en: {
        ...
        externalConfigs: 'External Raw Configs',
        ...
        welcomeTitle: 'MatiX Panel',
        welcomeSubtitle: 'Powered by BPB',
        welcomeText: 'Join our Telegram channel for updates, announcements and support.',
        welcomeJoin: 'Join Telegram Channel',
        welcomeSkip: 'Skip',
        tourSettingsDesc: 'Update, reset the admin password, or delete your panel from here.',
        tourTelegramBotDesc: 'Connect a Telegram bot to receive notifications and manage your panel from Telegram.',
        tourCommonDesc: 'Core proxy options such as UUID, ports and protocol behavior. Configure these first.',
        tourVlessTrojanDesc: 'Enable and fine-tune your VLESS and Trojan configs, including Proxy IP, ECH and custom CDN.',
        tourFragmentDesc: 'Xray Fragment settings help bypass restrictive networks by splitting traffic packets.',
        tourExternalConfigsDesc: 'Add your own external subscription links to merge with the generated configs.',
        tourWarpGeneralDesc: 'General Warp settings for routing traffic through Cloudflare Warp.',
        tourWarpProDesc: 'Advanced Warp options including noise modes for better obfuscation.',
        tourRoutingRulesDesc: 'Control which domains or apps are proxied, blocked, or bypassed.',
        tourImportExportDesc: 'Backup your settings or restore them from a file or remote URL.',
        tourNext: 'Next',
        tourPrev: 'Back',
        tourFinish: 'Finish',
        tourSkip: 'Skip tour'
    ...
    fa: {
        admin: 'مدیریت',
        last24h: 'درخواست‌های ۲۴ ساعت اخیر',
        settings: 'تنظیمات',
        telegramBot: 'ربات تلگرام',
        proxySettings: 'تنظیمات پروکسی',
        common: 'عمومی',
        vlessTrojan: 'وی‌لس - تروجان',
        fragment: 'فرگمنت Xray',
        externalConfigs: 'کانفیگ‌های خام خارجی',
        warpGeneral: 'وارپ عمومی',
        warpPro: 'وارپ پرو',
        routingRules: 'قوانین مسیریابی',
        importExport: 'درون‌ریزی - برون‌بری',
        subscriptions: 'لینک‌های اشتراک',
        dohTitle: 'دی‌ان‌اس روی HTTPS',
        myIp: 'آی‌پی من',
        supportedClients: 'اپلیکیشن‌های پشتیبانی‌شده',
        brandSubtitle: 'ساخته‌شده بر پایه‌ی BPB',
        welcomeTitle: 'پنل مطیکس',
        welcomeSubtitle: 'ساخته‌شده بر پایه‌ی BPB',
        welcomeText: 'برای دریافت آپدیت‌ها، اطلاعیه‌ها و پشتیبانی به کانال تلگرام ما بپیوندید.',
        welcomeJoin: 'عضویت در کانال تلگرام',
        welcomeSkip: 'رد کردن',
        tourSettingsDesc: 'از اینجا می‌تونید پنل رو آپدیت کنید، رمز مدیر رو ریست کنید یا پنل رو حذف کنید.',
        tourTelegramBotDesc: 'یه ربات تلگرام وصل کنید تا اعلان‌ها رو دریافت کنید و پنلتون رو از تلگرام مدیریت کنید.',
        tourCommonDesc: 'تنظیمات اصلی پروکسی مثل UUID، پورت‌ها و رفتار پروتکل. اول این‌ها رو تنظیم کنید.',
        tourVlessTrojanDesc: 'کانفیگ‌های VLESS و Trojan رو فعال و تنظیم کنید، شامل Proxy IP، ECH و CDN دلخواه.',
        tourFragmentDesc: 'تنظیمات Xray Fragment با تکه‌تکه کردن بسته‌ها به عبور از فیلترینگ سخت‌گیرانه کمک می‌کنه.',
        tourExternalConfigsDesc: 'لینک‌های اشتراک خارجی خودتون رو اضافه کنید تا با کانفیگ‌های تولیدشده ترکیب بشن.',
        tourWarpGeneralDesc: 'تنظیمات عمومی Warp برای مسیردهی ترافیک از طریق Cloudflare Warp.',
        tourWarpProDesc: 'تنظیمات پیشرفته‌ی Warp شامل حالت‌های نویز برای مخفی‌کاری بهتر.',
        tourRoutingRulesDesc: 'کنترل کنید کدوم دامنه‌ها یا اپ‌ها پروکسی، مسدود یا مستقیم بشن.',
        tourImportExportDesc: 'تنظیماتتون رو پشتیبان بگیرید یا از یه فایل/آدرس ریموت بازیابی کنید.',
        tourNext: 'بعدی',
        tourPrev: 'قبلی',
        tourFinish: 'پایان',
        tourSkip: 'رد کردن آموزش'
    }
};
...
async function checkVersion(panelVersion) {
    try {
        const res = await fetch('https://raw.githubusercontent.com/schooldars91-maker/Matixpanel/refs/heads/main/package.json', {
            cache: 'no-store'
        });
...
async function updatePanel(btn) {
    const confirm = await notify('confirm', 'Update MatiX Panel', [
        `MatiX Panel version ${globalThis.latestVersion} is now available!`,
        `Please read <a href='https://github.com/schooldars91-maker/Matixpanel/releases/latest' target='_blank' rel='noopener noreferrer'>Release notes</a> carefully before updating.`,
        'Are you sure?'
    ]);
    ...
    fetch('./panel/update-panel', { method: 'POST' })
        ...
        .catch(error => {
            notify('error', 'Update panel', ['Failed to update your MatiX Panel, please try again.']);
...
async function deletePanel(btn) {
    const confirm = await notify('confirm', 'Delete MatiX Panel', [
    ...
    fetch('./panel/delete-panel', { method: 'POST' })
        ...
        .catch(error => {
            notify('error', 'Delete panel', ['Failed to delete your MatiX Panel, please try again.']);
...
/* ===== Welcome Modal (Telegram promo) ===== */
function showWelcomeModal() {
    if (localStorage.getItem('matixWelcomeSeen')) {
        startTourIfNeeded();
        return;
    }
    ...
    const overlay = elm('div', { id: 'welcomeModal', className: 'modal' });
    const content = elm('div', { className: 'modal-content' });

    const titleBlock = elm('div', { className: 'welcome-title' }, [
        elm('h2', { textContent: dict.welcomeTitle }),
        elm('span', { textContent: dict.welcomeSubtitle })
    ]);

    const text = elm('p', { className: 'welcome-text', textContent: dict.welcomeText });

    const joinBtn = elm('a', {
        href: 'https://t.me/Imatix7',
        target: '_blank',
        rel: 'noopener noreferrer',
        className: 'button telegram'
    }, [
        createIcon('open_in_new'),
        elm('span', { textContent: dict.welcomeJoin })
    ]);

    const skipBtn = elm('button', {
        type: 'button',
        className: 'welcome-skip',
        textContent: dict.welcomeSkip,
        onclick: closeWelcomeModal
    });

    content.append(titleBlock, text, joinBtn, skipBtn);
...
function closeWelcomeModal() {
    ...
    localStorage.setItem('matixWelcomeSeen', '1');
    startTourIfNeeded();
}

/* ===== Onboarding Tour ===== */
const tourSteps = [
    { i18nHeading: 'settings', i18nDesc: 'tourSettingsDesc' },
    { i18nHeading: 'telegramBot', i18nDesc: 'tourTelegramBotDesc' },
    { i18nHeading: 'common', i18nDesc: 'tourCommonDesc' },
    { i18nHeading: 'vlessTrojan', i18nDesc: 'tourVlessTrojanDesc' },
    { i18nHeading: 'fragment', i18nDesc: 'tourFragmentDesc' },
    { i18nHeading: 'externalConfigs', i18nDesc: 'tourExternalConfigsDesc' },
    { i18nHeading: 'warpGeneral', i18nDesc: 'tourWarpGeneralDesc' },
    { i18nHeading: 'warpPro', i18nDesc: 'tourWarpProDesc' },
    { i18nHeading: 'routingRules', i18nDesc: 'tourRoutingRulesDesc' },
    { i18nHeading: 'importExport', i18nDesc: 'tourImportExportDesc' }
];
...
function startTourIfNeeded() {
    if (localStorage.getItem('matixTourSeen')) return;
    tourTargets = Array.from(document.querySelectorAll('.accordion-item'));
    if (tourTargets.length === 0) return;

    const overlay = elm('div', { id: 'tourOverlay' });
    const box = elm('div', { className: 'tour-highlight-box', id: 'tourHighlightBox' });
    const tooltip = elm('div', { className: 'tour-tooltip', id: 'tourTooltip' });
    overlay.append(box, tooltip);
    document.body.appendChild(overlay);
...
function renderTourStep() {
    ...
    const header = elm('div', { className: 'tour-tooltip-header' }, [
        elm('span', { className: 'tour-tooltip-step', textContent: String(tourIndex + 1) }),
        elm('strong', { textContent: dict[step.i18nHeading] || step.i18nHeading })
    ]);
    const desc = elm('div', { className: 'tour-tooltip-desc', textContent: dict[step.i18nDesc] || '' });

    const count = elm('span', {
        className: 'tour-tooltip-count',
        textContent: `${tourIndex + 1} / ${tourSteps.length}`
    });

    const actions = elm('div', { className: 'tour-tooltip-actions' });
    const skipBtn = elm('button', {
        type: 'button',
        className: 'tour-btn skip',
        textContent: dict.tourSkip,
        onclick: endTour
    });
    const prevBtn = elm('button', {
        type: 'button',
        className: 'tour-btn',
        textContent: dict.tourPrev,
        onclick: tourPrev
    });
    const nextBtn = elm('button', {
        type: 'button',
        className: 'tour-btn primary',
        textContent: tourIndex === tourSteps.length - 1 ? dict.tourFinish : dict.tourNext,
        onclick: tourNext
    });

    if (tourIndex > 0) actions.appendChild(prevBtn);
    actions.appendChild(nextBtn);

    const footer = elm('div', { className: 'tour-tooltip-footer' }, [count, actions]);

    tooltip.append(header, desc, skipBtn, footer);

    setTimeout(positionTourStep, 300);
}
...
function positionTourStep() {
    ...
    const tooltip = document.getElementById('tourTooltip');
    if (!target || !box || !tooltip) return;

    const rect = target.getBoundingClientRect();
    const pad = 6;

    box.style.top = `${rect.top - pad + window.scrollY}px`;
    box.style.left = `${rect.left - pad + window.scrollX}px`;
    ...
    if (tooltipLeft + 320 > window.innerWidth) {
        tooltipLeft = Math.max(12, window.innerWidth - 332);
    }
    if (tooltipTop + 200 > window.innerHeight + window.scrollY) {
        tooltipTop = rect.top + window.scrollY - 14;
...
function endTour() {
    ...
    localStorage.setItem('matixTourSeen', '1');
}
